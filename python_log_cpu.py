...
logging.basicConfig(
    ...
    level=logging.INFO,
    handlers=[
        RotatingFileHandler(log_file, maxBytes=5 * 1024 * 1024, backupCount=3),  # 5 MB per file, 3 backups
        logging.StreamHandler()  # Logs console par bhi dikhaye
    ]
)
...
def split_and_process_video(input_video, output_folder, clip_duration=30):  # Reduced clip duration
    os.makedirs(output_folder, exist_ok=True)
    logger.info(f"Processing video: {input_video}")
    
    with VideoFileClip(input_video) as video:  # Use 'with' to manage resources
        video_duration = video.duration
        num_clips = int(video_duration // clip_duration) + (1 if video_duration % clip_duration else 0)

        logger.info(f"Total clips to process: {num_clips}")
        ...
        for i in tqdm(range(num_clips), desc="Processing Clips", unit="clip"):
            ...
            logger.info(f"Writing subclip to: {temp_path}")
            subclip.write_videofile(temp_path, codec="libx264", audio_codec="aac", fps=15, bitrate="500k")  # Reduced FPS and bitrate
...
def resize_video(input_video, output_video):
    ...
    with VideoFileClip(input_video) as video:  # Use 'with' to manage resources
        ...
        final_video.write_videofile(output_video, fps=15, bitrate="500k", codec="libx264", audio_codec="aac")  # Reduced FPS and bitrate
...
def add_text_overlay(input_video, output_video, text):
    with VideoFileClip(input_video) as video:  # Use 'with' to manage resources
        ...
        final.write_videofile(output_video, fps=15, bitrate="500k", codec="libx264", audio_codec="aac")  # Reduced FPS and bitrate
